...
export async function registerRoutes(
  ...
  // Archive endpoints
  app.get("/api/archive/emails", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: "Unauthorized" });
      }
      const archived = await storage.getArchivedEmails(userId);
      res.json(archived);
    } catch (error) {
  ...
  app.post("/api/archive/emails", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: "Unauthorized" });
      }
      const { emailId, threadId, subject, from, snippet, date, isUnread } = req.body;
      if (!emailId) {
        return res.status(400).json({ error: "emailId is required" });
      }
      
      const isArchived = await storage.isEmailArchived(userId, emailId);
      if (isArchived) {
        return res.status(400).json({ error: "Email already archived" });
      }
      
      const archived = await storage.archiveEmail({
        userId,
        emailId,
        threadId,
        subject,
  ...
  app.delete("/api/archive/emails/:emailId", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      ...
      await storage.unarchiveEmail(userId, emailId);
      res.status(204).send();
    } catch (error) {
  ...
  app.get("/api/archive/slack", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: "Unauthorized" });
      }
      const archived = await storage.getArchivedSlackMessages(userId);
      res.json(archived);
    } catch (error) {
      console.error("Error fetching archived Slack messages:", error);
      res.status(500).json({ error: "Failed to fetch archived Slack messages" });
    }
  });

  app.post("/api/archive/slack", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: "Unauthorized" });
      }
      const { messageId, channelId, channelName, text, userName, timestamp, isDm } = req.body;
      if (!messageId) {
        return res.status(400).json({ error: "messageId is required" });
      }
      
      const isArchived = await storage.isSlackMessageArchived(userId, messageId);
      if (isArchived) {
        return res.status(400).json({ error: "Message already archived" });
      }
      
      const archived = await storage.archiveSlackMessage({
        userId,
        messageId,
        channelId,
        channelName,
      ...
      res.status(500).json({ error: "Failed to archive Slack message" });
    }
  });

  app.delete("/api/archive/slack/:messageId", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      ...
      await storage.unarchiveSlackMessage(userId, messageId);
      res.status(204).send();
    } catch (error) {
      console.error("Error unarchiving Slack message:", error);
      res.status(500).json({ error: "Failed to unarchive Slack message" });
    }
  });
