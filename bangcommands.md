...
| `!dbbackup`             | Database backup            | Run backup script to download production data   |
...
- **Purpose**: Download all production database data to a local JSON file
- **Behavior**: Runs the database backup script (`scripts/backup-database.ts`) which:
  - Connects to the production database with retry logic for cold starts
  - Exports all tables (organizations, users, students, classes, attendance, revenue, etc.)
  - Saves data to `backups/db-backup-TIMESTAMP.json`
  - Reports progress and final file size
- **Script Location**: `scripts/backup-database.ts`
- **Output Location**: `backups/` directory
- **Manual Execution**: `npx tsx scripts/backup-database.ts`
- **Use Cases**:
  - Creating offline backups of production data
